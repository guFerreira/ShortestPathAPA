...
class Experimento(ABC):
    ...
    def carrega_resultados(self):
        ...
        for l in f:
            ...
            if l[0] != "#":
                ...
                self.qnt_vertices.append(float(l.split(" ")[3]))
                self.qnt_arestas.append(float(l.split(" ")[4]))
        f.close()
    ...
    def plota_aproximacao(self):
        plt.plot(self.tamanhos_aproximados, self.aproximados, label=self.aproximacao_legenda,
                 color=self.aproximacao_cor_rgb)
...
class DijkstraAlgorithm(Experimento):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Trabalho APA - Dijkstra')
